...
const Testimonial = (props) => {
  ...
  const arrowLeft = () => {
    index === 0 ? setIndex(props.testimonialData.length - 1) : setIndex(index - 1)
  };

  const arrowRight = () => {
    index === props.testimonialData.length - 1 ? setIndex(0) : setIndex(index + 1)
  };

  const previous = () => {
    leftRef.current = arrowLeft();
  };

  const next = () => {
    rightRef.current = arrowRight();
  };
